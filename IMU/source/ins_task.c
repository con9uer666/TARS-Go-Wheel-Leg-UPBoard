...
void INS_Init(void)
{
    IMU_Param.scale[X_axis] = 1;
    IMU_Param.scale[Y_axis] = 1;
    IMU_Param.scale[Z_axis] = 1;
    IMU_Param.Yaw = 0;
    IMU_Param.Pitch = 0;
    IMU_Param.Roll = 0;
    IMU_Param.flag = 1;
...
static void IMU_Param_Correction(IMU_Param_t *param, float gyro[3], float accel[3])
{
    ...
    gyro[X_axis] = c_11 * gyro_temp[X_axis] +
              c_12 * gyro_temp[Y_axis] +
              c_13 * gyro_temp[Z_axis];
    gyro[Y_axis] = c_21 * gyro_temp[X_axis] +
              c_22 * gyro_temp[Y_axis] +
              c_23 * gyro_temp[Z_axis];
    gyro[Z_axis] = c_31 * gyro_temp[X_axis] +
              c_32 * gyro_temp[Y_axis] +
              c_33 * gyro_temp[Z_axis];

    float accel_temp[3];
    for (uint8_t i = 0; i < 3; i++)
        accel_temp[i] = accel[i];

    accel[X_axis] = c_11 * accel_temp[X_axis] +
               c_12 * accel_temp[Y_axis] +
               c_13 * accel_temp[Z_axis];
    accel[Y_axis] = c_21 * accel_temp[X_axis] +
               c_22 * accel_temp[Y_axis] +
               c_23 * accel_temp[Z_axis];
    accel[Z_axis] = c_31 * accel_temp[X_axis] +
               c_32 * accel_temp[Y_axis] +
               c_33 * accel_temp[Z_axis];
...
{
  return  (INS.Gyro[X_axis])/PI*180;
}
//Y轴角速度  PITCH
float User_Imu_Get_Y_Gyro(void)
{
  return  (INS.Gyro[Y_axis])/PI*180;
}
//z轴角速度  YAW
float User_Imu_Get_Z_Gyro(void)
{
  return  (INS.Gyro[Z_axis])/PI*180;
}

//X轴加速度 ROLL
float User_Imu_Get_X_Accell(void)
{
	return  (INS.Accel[X_axis]);
}
//y轴加速度
float User_Imu_Get_Y_Accell(void)
{
	return  (INS.Accel[Y_axis]);
}
//z轴加速度
float User_Imu_Get_Z_Accell(void)
{
	return  (INS.Accel[Z_axis]);
}
